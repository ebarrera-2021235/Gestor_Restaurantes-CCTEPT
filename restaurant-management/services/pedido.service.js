...
exports.createOrder = async (data) => {
    for (const item of data.items) {
        ...
        const pertenece = menuItem.restaurantes.some(
            (id) => id.toString() === data.restaurantId
        );
        if (!pertenece) {
            const error = new Error(`El plato ${menuItem._id} no pertenece al restaurante`);
...
exports.aplicarPromocionAPedido = async (orderId, promotionId) => {
    const { aplicarDescuento } = require("./promotion.service.js");
    const order = await Order.findById(orderId);
